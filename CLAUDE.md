...
| `cat_state.json` | personality: affection, traits, confidence facts, **user_profile + quiz** (answers), mood, persisted drives + affinity/trust/active_hours + **behavior_counts/secs**, `schema_version` (now **3**) |
...
She asks short questions (API-generated for uniqueness, static `QUESTIONS`
fallback) with clickable answers, at random in any mode (cooldown-gated). Answers
fill `user_profile`, nudge her traits, and feed the system prompt + musing
cadence. `maybe_ask`/`ask_now`/`poll_question`/`answer_question` mirror say/poll.
